...
static void publish_motor_event(const char *event)
{
    ...
    // Allocate if needed
    if (msg.data.capacity < len + 1) {
        msg.data.data = realloc(msg.data.data, len + 1);
        msg.data.capacity = len + 1;
    }

    memcpy(msg.data.data, event, len + 1);
    msg.data.size = len;
...
void app_main(void)
{
    ...
    xTaskCreate(micro_ros_spin_task, "uros_spin", 4096, NULL, 5, NULL);
